...
import { getInvoiceBusiness, PLATE_INVOICE_BUSINESS } from './invoiceConstants'
...
export function buildInvoiceEmail(options: {
  ...
  const business = getInvoiceBusiness()
  ...
  // Typography-only wordmark (Georgia/serif fallback for email clients) — no logo images.
  const html = `<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background:#f6f3ec;font-family:Georgia,'Times New Roman',serif;color:#14120e;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f6f3ec;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid #e4dccb;padding:32px;">
        <tr><td style="font-family:Georgia,'Times New Roman',serif;font-size:26px;letter-spacing:0.03em;color:#100f0c;">${PLATE_INVOICE_BUSINESS.name}</td></tr>
        ...
        <tr><td style="padding-top:28px;font-family:Helvetica,Arial,sans-serif;font-size:13px;color:#6f675d;line-height:1.5;">
          ${business.region}<br/>
          ${business.email}<br/>
          ${business.website}
        </td></tr>
